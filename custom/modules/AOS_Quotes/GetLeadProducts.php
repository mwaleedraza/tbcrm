...
while($rows = $GLOBALS['db']->fetchByAssoc($leads)){
  ...
  $subQuery = $GLOBALS['db']->query("SELECT `name` FROM tc_sub_products WHERE `id` = '".$sub_product_id."'  AND deleted=0");
  $subProducts = $GLOBALS['db']->fetchByAssoc($subQuery);
  // array_push($data, $subProducts);

  // Fetching Sub Sub Products
  $sub_subQuery = $GLOBALS['db']->query("SELECT `name` FROM tc_sub_products WHERE `id` = '".$sub_sub_product_id."'  AND deleted=0");
  $sub_subProducts = $GLOBALS['db']->fetchByAssoc($sub_subQuery);
  // array_push($data, $sub_subProducts);

  array_push($data,array("id" => $product_rows['id'], "product" => $product_rows['name'], "sub_product" => $subProducts['name'], "sub_sub_product" => $sub_subProducts['name']));
