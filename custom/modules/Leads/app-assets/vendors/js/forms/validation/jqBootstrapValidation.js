...
(function( $ ){
    ...
    var defaults = {
      ...
      filter: function () {
        // return jq331(this).is(":visible"); // only validate elements you can see
      ...
      init : function( options ) {
        ...
        var uniqueForms = $.unique(
          $siblingElements.map( function () {
            return jq331(this).parents("form")[0];
          }).toArray()
        );

        jq331(uniqueForms).bind("submit", function (e) {
          var $form = jq331(this);
          var warningsFound = 0;
        ...
        return this.each(function(){
          ...
          if (!$helpBlock.length && settings.options.autoAdd && settings.options.autoAdd.helpBlocks) {
              $helpBlock = jq331('<div class="help-block" />');
          ...
          $this.bind(
            ...
            function (e, params) {
              ...
              $controlGroup.find("input,textarea,select").each(function (i, el) {
                ...
                $.each(jq331(el).triggerHandler("validation.validation", params), function (j, message) {
                  errorsFound.push(message);
                });
                if (errorsFound.length > oldCount) {
                  jq331(el).attr("aria-invalid", "true");
                } else {
                  var original = $this.data("original-aria-invalid");
                  jq331(el).attr("aria-invalid", (original !== undefined ? original : false));
                }
              });
      ...
      hasErrors: function() {
        ...
        this.find('input,select,textarea').add(this).each(function (i, el) {
          errorMessages = errorMessages.concat(
            jq331(el).triggerHandler("getValidators.validation") ? jq331(el).triggerHandler("validation.validation", {submitting: true}) : []
          );
        });
    ...
    var hasParent = !!(parent = $this.parents("form").first()) || !!(parent = $this.parents(".control-group").first());
      ...
      if (hasParent) {
        value = parent.find("input[type='checkbox'][name='" + $this.attr("name") + "']:checked").map(function (i, el) { return jq331(el).val(); }).toArray().join(",");
      }
        }
        if (type === "radio") {
            value = (jq331('input[name="' + $this.attr("name") + '"]:checked').length > 0 ? value : "");
      if (hasParent) {
        value = parent.find("input[type='radio'][name='" + $this.attr("name") + "']:checked").map(function (i, el) { return jq331(el).val(); }).toArray().join(",");
      }
        }
        return value;
    };
...
$.fn.jqBootstrapValidation = function( method ) {
  ...
  $.jqBootstrapValidation = function (options) {
    jq331(":input").not("[type=image],[type=submit]").jqBootstrapValidation.apply(this,arguments);
  };
