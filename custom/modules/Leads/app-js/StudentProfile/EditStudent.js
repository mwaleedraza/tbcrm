var EditLead = {
    ...
    setupSelect2DropdownEventListeners: function () {
		...
		jq331('#from_subagent').on('select2:select', function (e) {
			var data = e.params.data;
			if (data.id == "Yes") {
                jq331('#igec_subagents_id').prop('disabled', false);
				jq331('#subagban').hide('slow');
			}else{
				jq331('#igec_subagents_id').prop('disabled', true);
...
function gotoRecord(id){
    $("#dialogFor").dialog("close");
    window.location="index.php?module=Contacts&action=DetailView&record="+id;
}
function checkDuplicateEmail(){
    ...
    if(email !== ''){
        $.ajax({
            url: 'index.php?module=Contacts&action=checkDupliacteEntry',
            type: 'POST',
