var ManageInstStaff = {
    manageStaffClicked: function (obj) {
		...
		var stafftmpl = document.getElementById('staffTpl').content.cloneNode(true);
		$('#moredatadivwrapper').show();
		$('#institutesTabDiv').addClass('disabled1');
        $('#moredatadiv').append('<h3 class="info">Manage Staff</h3>');
        $('#moredatadiv').append(stafftmpl);
		getStaff(accountId);
	...
	saveNewStaffClicked: function (obj) {
		igec.blockFullPage();
			...
			jq331.ajax({
				type: "POST",
				url: "index.php?module=igec_sponsor_organization&action=saveNewAccountStaff&sugar_body_only=true",
				data: formdata,
				...
				success: function (data) {
					if (typeof data != 'undefined' && data != '') {
						...
						jq331('#account_id').val(account_id);
						delstaff();
						jq331.unblockUI();
					}
...
function getStaff(rec){
	...
	jq331.ajax({
		type: "POST",
		url: "index.php?module=igec_sponsor_organization&action=getAccountStaffData&sugar_body_only=true",
		data: {record :rec},
